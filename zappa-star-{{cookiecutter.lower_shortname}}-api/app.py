...
def setup_routes(app):
    ...
    def current_games():
        ...
        if mode < 0:
            ...
            sdg = fetch_getter("star-{{cookiecutter.lower_shortname}}", "season_day", datadir, season0, day0)
            ...
            if series is None:
                raise APIError()
            getter = fetch_getter("star-{{cookiecutter.lower_shortname}}", "postseason_day", datadir, season0, series, 1)
            ...
            getter = fetch_getter("star-{{cookiecutter.lower_shortname}}", "postseason_day", datadir, season0, series, series_day)
            games = getter.get_postseason_day_game_data_slim()
            games.sort(key=lambda x: x["description"])
    ...
    def games(season0, day0):
        ...
        if season0 < current_season0:
            dg = fetch_getter("star-{{cookiecutter.lower_shortname}}", "season_day", datadir, season0, day0)
    ...
    def game(gameid, event=None, context=None):
        gg = fetch_getter("star-{{cookiecutter.lower_shortname}}", "game", datadir, gameid)
    ...
    def a_season(season0):
        ...
        if season0 < current_season0:
            ...
            sg = fetch_getter("star-{{cookiecutter.lower_shortname}}", "season", datadir, season0)
            ...
            if mode < 10:
                ...
                # Regular season, use StarIISeasonPrecedingDayGetter
                spd = fetch_getter("star-{{cookiecutter.lower_shortname}}", "season_preceding_day", datadir, season0, day0)
    ...
    def a_postseason(season0):
        ...
        - one key per series (LDS, LCS, SCS)
        ...
        if season0 < current_season0:
            ...
            pg = fetch_getter("star-{{cookiecutter.lower_shortname}}", "postseason", datadir, season0)
            ...
            if mode < 20:
                ...
                if series is None:
                    raise FlaskError()
                ppdg = fetch_getter("star-{{cookiecutter.lower_shortname}}", "postseason_preceding_day", datadir, season0, series, 1)
                ...
                ppdg = fetch_getter(
                    "star-{{cookiecutter.lower_shortname}}", "postseason_preceding_day", datadir, season0, series, series_day + 1
                )
                result = ppdg.get_postseason_precedingdays_game_data_slim()
                for s in result.keys():
                    for i, _ in enumerate(result[s]):
                ...
                pdg = fetch_getter("star-{{cookiecutter.lower_shortname}}", "postseason", datadir, season0)
    ...
    def a_seeds(season0):
        ...
        if season0 < current_season0:
            ...
            sg = fetch_getter("star-{{cookiecutter.lower_shortname}}", "seed", datadir, season0)
    ...
    def a_champion(season0):
        """Returns the Star II Cup champion for the specified season"""
        if season0 is None:
            season0 = g.timekeeper.get_current_season()
        if season0 < 0:
            raise InvalidSeasonError()
        ...
        if season0 < current_season0:
            ...
            cg = fetch_getter("star-{{cookiecutter.lower_shortname}}", "champion", datadir, season0)
    ...
    def a_teams(season0):
        ...
        if season0 <= current_season0:
            ...
            tg = fetch_getter("star-{{cookiecutter.lower_shortname}}", "teams", datadir, season0)
    ...
    def a_records(season0):
        """
        Return WL record info for the given team in the given season.
        ...
        If StarIIRecordsGetter receives a day > 48, it returns the records
        ...
        if season0 < current_season0:
            ...
            rg = fetch_getter("star-{{cookiecutter.lower_shortname}}", "records", datadir, season0)
            ...
            if mode > 10 and mode < 30:
                ...
                rg = fetch_getter("star-{{cookiecutter.lower_shortname}}", "records", datadir, season0, current_day0 + 1)
                wlrecords = rg.get_records_data(use_abbr=False)
                return jsonify(wlrecords)
            else:
                rg = fetch_getter("star-{{cookiecutter.lower_shortname}}", "records", datadir, season0, current_day0)
                wlrecords = rg.get_records_data(use_abbr=False)
                return jsonify(wlrecords)
    ...
    def a_standings_day(season0, day0):
        ...
        if season0 < current_season0:
            ...
            rg = fetch_getter("star-{{cookiecutter.lower_shortname}}", "records", datadir, season0, day0 + 1)
            ...
            if day0 < current_day0 or mode >= 20:
                ...
                if mode >= 20:
                    ...
                    rg = fetch_getter("star-{{cookiecutter.lower_shortname}}", "records", datadir, season0, day0)
            else:
...
class Timekeeper(object):
    ...
    def get_current_day(self):
        ...
        if mode < 0:
            ...
            lds_length = self.get_postseason_series_length(season0, "LDS")
            lcs_length = self.get_postseason_series_length(season0, "LCS")
            scs_length = self.get_postseason_series_length(season0, "SCS")
            return dps + lds_length + lcs_length + scs_length
    ...
    def get_site_mode(self):
        ...
        # star cup series start
        diffscss = self.get_marker_diff_scs_start()
        if diffscss < 0:
            # Star Cup (SCS) is about to begin
            return {
                "mode": 23,
                "season": self.get_current_season(),
                "start": -1 * diffscss,
        ...
        # star cup series end
        diffscse = self.get_marker_diff_scs_end()
        if diffscse < 0:
            # Star Cup is in progress
            return {
                "mode": 33,
                "season": self.get_current_season(),
                "elapsed": diffscss,
    ...
    def prepare_current_season_marker_pool(self):
        ...
        self.postseason_lds_start = self.season_start + timedelta(hours=24)
        ...
        # starts 6 hours after postseason starts
        # (enough time for LDS + 1 hour siesta)
        self.postseason_lcs_start = self.postseason_lds_start + timedelta(hours=6)
        ...
        # starts 48 hours after season start
        self.postseason_scs_start = self.season_start + timedelta(hours=48)

        # Star Cup ends 1 hour after last game
        scs_hours = self.get_postseason_series_length(season0, "SCS")
        self.postseason_scs_end = self.postseason_scs_start + timedelta(hours=scs_hours)
    ...
    def get_marker_diff_scs_start(self):
        dtnow = datetime.now(self.time_zone)
        diffscss = (dtnow - self.postseason_scs_start).total_seconds()
        return int(diffscss)

    def get_marker_diff_scs_end(self):
        dtnow = datetime.now(self.time_zone)
        diffscse = (dtnow - self.postseason_scs_end).total_seconds()
        return int(diffscse)
    ...
    def get_postseason_series_length(self, season0, series):
        getter = fetch_getter("star-{{cookiecutter.lower_shortname}}", "postseason_length", datadir, season0, series)
